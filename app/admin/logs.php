...
<head>
    ...
    <style>
        ...
        .pagination .page-link { color: var(--primary-color); border: none; margin: 0 3px; border-radius: 5px; }
        .pagination .page-item.active .page-link { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
        ...
        <div class="main-container">
                ...
                <form method="GET" class="row g-3">
                    <div class="col-md-3">
                        ...
                        <select name="action_type" class="form-select">
                            ...
                            <option value="other" <?php echo $actionType=='other'?'selected':''; ?>>其他</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">搜索</label>
                        <input type="text" name="search" class="form-control" placeholder="用户名/操作/描述" value="<?php echo htmlspecialchars($search); ?>">
                    </div>
            ...
            <div class="log-table">
                <table class="table table-hover mb-0">
                    <thead><tr><th style="width:80px;">ID</th><th style="width:120px;">类型</th><th style="width:150px;">用户</th><th>操作</th><th>描述</th><th style="width:120px;">IP地址</th><th style="width:100px;">结果</th><th style="width:150px;">时间</th></tr></thead>
                    <tbody>
                        <?php if (empty($logs)): ?>
                        <tr><td colspan="8"><div class="empty-state"><i class="fas fa-inbox"></i><h5>暂无日志记录</h5><p>系统操作日志将显示在这里</p></div></td></tr>
                        <?php else: foreach ($logs as $log): $typeInfo = $actionTypeLabels[$log['action_type']] ?? $actionTypeLabels['other']; ?>
                        <tr>
                            <td><?php echo $log['id']; ?></td>
                            <td><span class="badge <?php echo $typeInfo['class']; ?> action-badge"><?php echo $typeInfo['label']; ?></span></td>
                            <td><?php echo htmlspecialchars($log['username'] ?? '未知'); ?></td>
                            <td><?php echo htmlspecialchars($log['action']); ?></td>
                            <td><?php echo htmlspecialchars($log['description'] ?? '-'); ?></td>
                            <td><?php echo htmlspecialchars($log['ip'] ?? '-'); ?></td>
                            <td><?php if ($log['result']==1): ?><span class="result-success"><i class="fas fa-check-circle"></i> 成功</span><?php else: ?><span class="result-fail"><i class="fas fa-times-circle"></i> 失败</span><?php endif; ?></td>
            ...
            <nav class="mt-4"><ul class="pagination justify-content-center">
                <?php if ($page > 1): ?><li class="page-item"><a class="page-link" href="?page=<?php echo $page-1; ?>&action_type=<?php echo urlencode($actionType); ?>&search=<?php echo urlencode($search); ?>&date_from=<?php echo urlencode($dateFrom); ?>&date_to=<?php echo urlencode($dateTo); ?>"><i class="fas fa-chevron-left"></i></a></li><?php endif; ?>
                <?php for ($i = max(1, $page-2); $i <= min($totalPages, $page+2); $i++): ?><li class="page-item <?php echo $i==$page?'active':''; ?>"><a class="page-link" href="?page=<?php echo $i; ?>&action_type=<?php echo urlencode($actionType); ?>&search=<?php echo urlencode($search); ?>&date_from=<?php echo urlencode($dateFrom); ?>&date_to=<?php echo urlencode($dateTo); ?>"><?php echo $i; ?></a></li><?php endfor; ?>
                <?php if ($page < $totalPages): ?><li class="page-item"><a class="page-link" href="?page=<?php echo $page+1; ?>&action_type=<?php echo urlencode($actionType); ?>&search=<?php echo urlencode($search); ?>&date_from=<?php echo urlencode($dateFrom); ?>&date_to=<?php echo urlencode($dateTo); ?>"><i class="fas fa-chevron-right"></i></a></li><?php endif; ?>
            </ul></nav>
            <?php endif; ?>
